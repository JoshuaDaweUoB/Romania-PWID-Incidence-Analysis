...
# hiv_test_20xx columns based on midpoint_year and hiv_test_rslt
for (year in required_years) {
  column_name <- paste0("hiv_test_", year)
...
two_yearly_results_midpoint <- midpoint_dataframe_long %>%
  ...
  summarise(
    total_hiv_infections = sum(hiv_test_rslt, na.rm = TRUE),
    total_person_years = sum(time_at_risk, na.rm = TRUE),
    incidence_rate = (total_hiv_infections / total_person_years) * 100,
    lower_bound = (total_hiv_infections / total_person_years) * 100 - 
                  1.96 * sqrt(total_hiv_infections / (total_person_years^2)) * 100,
...
midpoint_dataframe <- midpoint_dataframe %>%
  mutate(
    midpoint_year = ifelse(hiv_test_rslt == 0, NA, midpoint_year)
  )
...
yearly_data <- midpoint_dataframe %>%
  group_by(year) %>%
  summarise(
    cases = sum(hiv_test_rslt, na.rm = TRUE),
...
processed_dataframes_hiv_s3 <- lapply(processed_dataframes_hiv, function(df) {
...
# save _s3 dataframes
saveRDS(processed_dataframes_hiv_s3, file = "processed_dataframes_hiv_s3.rds")

# load _s3 data
processed_dataframes_hiv_s3 <- readRDS("processed_dataframes_hiv_s3.rds")

# list for processed dataframes
processed_dataframes_hiv_long_s3 <- list()
...
# loop over processed_dataframes_hiv_s3
for (i in 1:length(processed_dataframes_hiv_s3)) {
  cat("Processing dataframe", i, "of", length(processed_dataframes_hiv_s3), "\n")
  
  # process each dataframe
  processed_dataframes_hiv_long_s3[[i]] <- process_dataframe(processed_dataframes_hiv_s3[[i]])
}

# save long format processed dataframes to a file with the suffix _s3
saveRDS(processed_dataframes_hiv_long_s3, file = "processed_dataframes_long_hiv_s3.rds")

# list for all 10000 dataframes
all_two_yearly_results_hiv <- list()

# loop over 10000 dataframes
for (i in 1:length(processed_dataframes_hiv_long_s3)) {
  cat("Processing dataframe", i, "of", length(processed_dataframes_hiv_long_s3), "\n")
  
  # load dataframe
  midpoint_dataframe <- processed_dataframes_hiv_long_s3[[i]]
  
  # midpoint_year with NA if hiv_test_rslt is negative
  midpoint_dataframe <- midpoint_dataframe %>%
    mutate(
      midpoint_year = ifelse(hiv_test_rslt == 0, NA, midpoint_year)
    )
  ...
  # save results
  all_two_yearly_results_hiv[[i]] <- two_yearly_results
}

# combine into one df
combined_two_yearly_results_hiv_s3 <- bind_rows(all_two_yearly_results_hiv, .id = "iteration")

# save to a csv file
write.csv(combined_two_yearly_results_hiv_s3, "combined_two_yearly_results_hiv_s3.csv", row.names = FALSE)

# view results
View(combined_two_yearly_results_hiv_s3)

# incidence trends over time

# two-yearly intervals and calculate the median and percentiles
incidence_trends_hiv_s3 <- combined_two_yearly_results_hiv_s3 %>%
  group_by(two_year_interval) %>%
  summarise(
    ...
    median_total_hiv_infections = median(total_hiv_infections, na.rm = TRUE)
  )

# incidence trends
print(incidence_trends_hiv_s3)
View(incidence_trends_hiv_s3)

# save incidence trends to a csv file
write.csv(incidence_trends_hiv_s3, "incidence_trends_hiv_s3.csv", row.names = FALSE)

# plot for the incidence trends
hiv_incidence_trends_plot <- ggplot(incidence_trends_hiv_s3, aes(x = two_year_interval, y = median_incidence_rate)) +
  geom_line(group = 1, color = "gray", linewidth = 0.8, linetype = "solid") +
  geom_point(shape = 18, size = 4, color = "gray") +
  geom_errorbar(aes(ymin = lower_bound, ymax = upper_bound), width = 0.1, color = "black", size = 0.8) +
  theme_minimal(base_size = 14) +
